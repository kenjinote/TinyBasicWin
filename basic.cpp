/*
TOYOSHIKI TinyBASIC V1.0
Reference source
(C)2012 Tetsuya Suzuki, All rights reserved.
*/

#pragma comment(lib, "shlwapi")

// Compiler requires description
#include <SDKDDKVer.h>
#include <stdio.h>
#include <tchar.h>
#include <windows.h>
#include <shlwapi.h>
#include <conio.h>
#include <stdlib.h>
#include "basic.h"

// exception search macro
#define nospacea(c) sstyle(c, i_nsa, sizeof(i_nsa))
#define nospaceb(c) sstyle(c, i_nsb, sizeof(i_nsb))

// Return random number
int getrnd(int value) {
	return (rand() % value) + 1;
}

// exception search function
char sstyle(WCHAR code, const WCHAR *table, WCHAR count) {
	while (count--)
		if (code == table[count])
			return 1;
	return 0;
}

// Error messages
const WCHAR* errmsg[] = {
	L"OK",
	L"Devision by zero",
	L"Overflow",
	L"Subscript out of range",
	L"Icode buffer full",
	L"List full",
	L"GOSUB too many nested",
	L"RETURN stack underflow",
	L"FOR too many nested",
	L"NEXT without FOR",
	L"NEXT without counter",
	L"NEXT mismatch FOR",
	L"FOR without variable",
	L"FOR without TO",
	L"LET without variable",
	L"IF without condition",
	L"Undefined line number",
	L"\'(\' or \')\' expected",
	L"\'=\' expected",
	L"Illegal command",
	L"Syntax error",
	L"Internal error",
	L"Abort by [ESC]"
};

// Keyword table
const WCHAR *kwtbl[] = {
	L"GOTO", L"GOSUB", L"RETURN",
	L"FOR", L"TO", L"STEP", L"NEXT",
	L"IF", L"REM", L"STOP",
	L"INPUT", L"PRINT", L"LET",
	L",", L";",
	L"-", L"+", L"*", L"/", L"(", L")",
	L">=", L"#", L">", L"=", L"<=", L"<",
	L"@", L"RND", L"ABS", L"SIZE",
	L"LIST", L"RUN", L"NEW"
};

// Error code assignment
enum {
	ERR_OK,
	ERR_DIVBY0,
	ERR_VOF,
	ERR_SOR,
	ERR_IBUFOF, ERR_LBUFOF,
	ERR_GSTKOF, ERR_GSTKUF,
	ERR_LSTKOF, ERR_LSTKUF,
	ERR_NEXTWOV, ERR_NEXTUM, ERR_FORWOV, ERR_FORWOTO,
	ERR_LETWOV, ERR_IFWOC,
	ERR_ULN,
	ERR_PAREN, ERR_VWOEQ,
	ERR_COM,
	ERR_SYNTAX,
	ERR_SYS,
	ERR_ESC
};

// Standard C libraly (about) same functions
WCHAR c_toupper(WCHAR c) { return(c <= L'z' && c >= L'a' ? c - 32 : c); }
WCHAR c_isprint(WCHAR c) { return(c >= 32 && c <= 126); }
WCHAR c_isspace(WCHAR c) { return(c == L' ' || (c <= 13 && c >= 9)); }
WCHAR c_isdigit(WCHAR c) { return(c <= L'9' && c >= L'0'); }
WCHAR c_isalpha(WCHAR c) { return ((c <= L'z' && c >= L'a') || (c <= L'Z' && c >= L'A')); }

void CBasic::c_puts(LPCWSTR s)
{
	SendMessageW(m_hOutputEditWnd, EM_REPLACESEL, 0, (LPARAM)s);
}

void CBasic::c_gets(int nLineIndex)
{
	lbuf[0] = _countof(lbuf);
	SendMessage(m_hInputEditWnd, EM_GETLINE, nLineIndex, (LPARAM)lbuf);
}

// Print numeric specified columns
void CBasic::putnum(int value, int d)
{
	wsprintf(lbuf, TEXT("%d"), value);
	SendMessage(m_hOutputEditWnd, EM_REPLACESEL, 0, (LPARAM)lbuf);
}

// Convert token to i-code
// Return byte length or 0
WCHAR CBasic::toktoi() {
	WCHAR i; // Loop counter(i-code sometime)
	WCHAR len = 0; //byte counter
	WCHAR* pkw = 0; // Temporary keyword pointer
	WCHAR* ptok; // Temporary token pointer
	WCHAR* s = lbuf; // Pointer to charactor at line buffer
	WCHAR c; // Surround the string character, " or '
	int value; //numeric
	int tmp; //numeric for overflow check
	while (*s) {
		while (c_isspace(*s)) s++; // Skip space
								   //Try keyword conversion
		for (i = 0; i < SIZE_KWTBL; i++) {
			pkw = (WCHAR*)kwtbl[i]; // Point keyword
			ptok = s; // Point top of command line
					  // Compare 1 keyword
			while ((*pkw != 0) && (*pkw == c_toupper(*ptok))) {
				pkw++;
				ptok++;
			}
			if (*pkw == 0) {// Case success
				if (len >= SIZE_IBUF - 1) {// List area full
					err = ERR_IBUFOF;
					return 0;
				}
				// i have i-code
				ibuf[len++] = i;
				s = ptok;
				break;
			}
		}
		// Case statement needs an argument except numeric, valiable, or strings
		if (i == I_REM) {
			while (c_isspace(*s)) s++; // Skip space
			ptok = s;
			for (i = 0; *ptok++; i++); // Get length
			if (len >= SIZE_IBUF - 2 - i) {
				err = ERR_IBUFOF;
				return 0;
			}
			ibuf[len++] = i; // Put length
			while (i--) { // Copy strings
				ibuf[len++] = *s++;
			}
			break;
		}
		if (*pkw == 0)
			continue;
		ptok = s; // Point top of command line
		// Try numeric conversion
		if (c_isdigit(*ptok)) {
			value = 0;
			tmp = 0;
			do {
				tmp = 10 * value + *ptok++ - '0';
				if (value > tmp) {
					err = ERR_VOF;
					return 0;
				}
				value = tmp;
			} while (c_isdigit(*ptok));

			if (len >= SIZE_IBUF - 3) {
				err = ERR_IBUFOF;
				return 0;
			}
			s = ptok;
			ibuf[len++] = I_NUM;
			ibuf[len++] = value & 255;
			ibuf[len++] = value >> 8;
		}
		else

			// Try string conversion
			if (*s == '\"' || *s == '\'') {// If start of string
				c = *s++;
				ptok = s;
				for (i = 0; (*ptok != c) && c_isprint(*ptok); i++) // Get length
					ptok++;
				if (len >= SIZE_IBUF - 1 - i) { // List area full
					err = ERR_IBUFOF;
					return 0;
				}
				ibuf[len++] = I_STR; // Put i-code
				ibuf[len++] = i; // Put length
				while (i--) { // Put string
					ibuf[len++] = *s++;
				}
				if (*s == c) s++; // Skip " or '
			}
			else
				// Try variable conversion
				if (c_isalpha(*ptok)) {
					if (len >= SIZE_IBUF - 2) {
						err = ERR_IBUFOF;
						return 0;
					}
					if (len >= 4 && ibuf[len - 2] == I_VAR && ibuf[len - 4] == I_VAR) { // Case series of variables
						err = ERR_SYNTAX; // Syntax error
						return 0;
					}
					ibuf[len++] = I_VAR; // Put i-code
					ibuf[len++] = c_toupper(*ptok) - 'A'; // Put index of valiable area
					s++;
				}
				else
					// Nothing mutch
				{
					err = ERR_SYNTAX;
					return 0;
				}
	}
	ibuf[len++] = I_EOL; // Put end of line
	return len; // Return byte length
}

// Get line number by line pointer
int getlineno(WCHAR *lp) {
	if (*lp == 0) //end of list
		return 32767;// max line bumber
	return *(lp + 1) | *(lp + 2) << 8;
}

// Search line by line number
WCHAR* CBasic::getlp(int lineno) {
	WCHAR *lp;

	for (lp = listbuf; *lp; lp += *lp)
		if (getlineno(lp) >= lineno)
			break;
	return lp;
}

// Return free memory size
int CBasic::getsize() {
	WCHAR* lp;

	for (lp = listbuf; *lp; lp += *lp);
	return (int)(listbuf + SIZE_LIST - lp - 1);
}

// Insert i-code to the list
// Preconditions to do *ibuf = len
void CBasic::inslist() {
	WCHAR *insp;
	WCHAR *p1, *p2;
	WCHAR len;

	if (getsize() < *ibuf) {
		err = ERR_LBUFOF; // List buffer overflow
		return;
	}

	insp = getlp(getlineno(ibuf));

	if (getlineno(insp) == getlineno(ibuf)) {// line number agree
		p1 = insp;
		p2 = p1 + *p1;
		while ((len = *p2) != 0) {
			while (len--)
				*p1++ = *p2++;
		}
		*p1 = 0;
	}

	// Case line number only
	if (*ibuf == 4)
		return;

	// Make space
	for (p1 = insp; *p1; p1 += *p1);
	INT_PTR len2 = p1 - insp + 1;
	p2 = p1 + *ibuf;
	while (len2--)
		*p2-- = *p1--;

	// Insert
	len = *ibuf;
	p1 = insp;
	p2 = ibuf;
	while (len--)
		*p1++ = *p2++;
}

//Listing 1 line of i-code
void CBasic::putlist(WCHAR* ip) {
	WCHAR i;

	while (*ip != I_EOL) {
		// Case keyword
		if (*ip < SIZE_KWTBL) {
			c_puts(kwtbl[*ip]);
			if (!nospacea(*ip))
				c_putch(' ');
			if (*ip == I_REM) {
				ip++;
				i = *ip++;
				while (i--) {
					c_putch(*ip++);
				}
				return;
			}
			ip++;
		}
		else

			// Case numeric
			if (*ip == I_NUM) {
				ip++;
				putnum(*ip | *(ip + 1) << 8, 0);
				ip += 2;
				if (!nospaceb(*ip)) c_putch(' ');
			}
			else

				// Case variable
				if (*ip == I_VAR) {
					ip++;
					c_putch((WCHAR)((*ip++) + 'A'));
					if (!nospaceb(*ip)) c_putch(' ');
				}
				else

					// Case string
					if (*ip == I_STR) {
						WCHAR c;

						c = L'\"';
						ip++;
						for (i = *ip; i; i--)
							if (*(ip + i) == L'\"') {
								c = L'\'';
								break;
							}

						c_putch(c);
						i = *ip++;
						while (i--) {
							c_putch(*ip++);
						}
						c_putch(c);
						if (*ip == I_VAR)
							c_putch(L' ');
					}

		// Nothing match, I think, such case is impossible
					else {
						err = ERR_SYS;
						return;
					}
	}
}

// Get argument in parenthesis
int CBasic::getparam() {
	int value;

	if (*cip != I_OPEN) {
		err = ERR_PAREN;
		return 0;
	}
	cip++;
	value = iexp();
	if (err)
		return 0;

	if (*cip != I_CLOSE) {
		err = ERR_PAREN;
		return 0;
	}
	cip++;

	return value;
}

// Get value
int CBasic::ivalue() {
	int value = 0;

	switch (*cip) {
	case I_NUM:
		cip++;
		value = *cip | *(cip + 1) << 8;
		cip += 2;
		break;
	case I_PLUS:
		cip++;
		value = ivalue();
		break;
	case I_MINUS:
		cip++;
		value = 0 - ivalue();
		break;
	case I_VAR:
		cip++;
		value = var[*cip++];
		break;
	case I_OPEN:
		value = getparam();
		break;
	case I_ARRAY:
		cip++;
		value = getparam();
		if (err)
			break;
		if (value >= SIZE_ARRY) {
			err = ERR_SOR;
			break;
		}
		value = arr[value];
		break;
	case I_RND:
		cip++;
		value = getparam();
		if (err)
			break;
		value = getrnd(value);
		break;
	case I_ABS:
		cip++;
		value = getparam();
		if (err)
			break;
		if (value < 0)
			value *= -1;
		break;
	case I_SIZE:
		cip++;
		if ((*cip != I_OPEN) || (*(cip + 1) != I_CLOSE)) {
			err = ERR_PAREN;
			break;
		}
		cip += 2;
		value = getsize();
		break;

	default:
		err = ERR_SYNTAX;
		break;
	}
	return value;
}

// multiply or divide calculation
int CBasic::imul()
{
	int value = ivalue(), tmp;

	if (err)
		return -1;

	while (1)
	{
		switch (*cip)
		{
		case I_MUL:
			cip++;
			tmp = ivalue();
			value *= tmp;
			break;
		case I_DIV:
			cip++;
			tmp = ivalue();
			if (tmp == 0) {
				err = ERR_DIVBY0;
				return -1;
			}
			value /= tmp;
			break;
		default:
			return value;
		}
	}
}

// add or subtract calculation
int CBasic::iplus() {
	int value, tmp;

	value = imul();
	if (err)
		return -1;

	while (1)
		switch (*cip) {
		case I_PLUS:
			cip++;
			tmp = imul();
			value += tmp;
			break;
		case I_MINUS:
			cip++;
			tmp = imul();
			value -= tmp;
			break;
		default:
			return value;
		}
}

// The parser
int CBasic::iexp() {
	int value, tmp;

	value = iplus();
	if (err)
		return -1;

	// conditional expression 
	while (1)
		switch (*cip) {
		case I_EQ:
			cip++;
			tmp = iplus();
			value = (value == tmp);
			break;
		case I_SHARP:
			cip++;
			tmp = iplus();
			value = (value != tmp);
			break;
		case I_LT:
			cip++;
			tmp = iplus();
			value = (value < tmp);
			break;
		case I_LTE:
			cip++;
			tmp = iplus();
			value = (value <= tmp);
			break;
		case I_GT:
			cip++;
			tmp = iplus();
			value = (value > tmp);
			break;
		case I_GTE:
			cip++;
			tmp = iplus();
			value = (value >= tmp);
			break;
		default:
			return value;
		}
}

// PRINT handler
void CBasic::iprint() {
	int value;
	int len;
	WCHAR i;

	len = 0;
	while (*cip != I_SEMI && *cip != I_EOL) {
		switch (*cip) {
		case I_STR:
			cip++;
			i = *cip++;
			while (i--)
				c_putch(*cip++);
			break;
		case I_SHARP:
			cip++;
			len = iexp();
			if (err)
				return;
			break;
		default:
			value = iexp();
			if (err)
				return;
			putnum(value, len);
			break;
		}

		if (*cip == I_COMMA) {
			cip++;
			if (*cip == I_SEMI || *cip == I_EOL)
				return;
		}
		else {
			if (*cip != I_SEMI && *cip != I_EOL) {
				err = ERR_SYNTAX;
				return;
			}
		}
	}
	newline();
}

// Variable assignment handler
void CBasic::ivar() {
	int value;
	int index;

	index = *cip++;
	if (*cip != I_EQ) {
		err = ERR_VWOEQ;
		return;
	}
	cip++;

	value = iexp();
	if (err)
		return;

	var[index] = value;
}

// Array assignment handler
void CBasic::iarray() {
	int value;
	int index;

	index = getparam();
	if (err)
		return;

	if (index >= SIZE_ARRY) {
		err = ERR_SOR;
		return;
	}

	if (*cip != I_EQ) {
		err = ERR_VWOEQ;
		return;
	}
	cip++;

	value = iexp();
	if (err)
		return;

	arr[index] = value;
}

// LET handler
void CBasic::ilet() {
	switch (*cip) {
	case I_VAR:
		cip++;
		ivar(); // Variable assignment
		break;
	case I_ARRAY:
		cip++;
		iarray(); // Array assignment
		break;
	default:
		err = ERR_LETWOV;
		break;
	}
}

// Execute a series of i-code
WCHAR* CBasic::iexe()
{
	int lineno; //line number
	WCHAR* lp; //temporary line pointer
	int index, vto, vstep; // FOR-NEXT items
	int condition; //IF condition
	while (*cip != I_EOL)
	{
		if (m_bAbort)
		{
			err = ERR_ESC;
			return NULL;
		}
		switch (*cip)
		{
		case I_GOTO:
			cip++;
			lineno = iexp(); // get line number
			if (err)
				break;
			lp = getlp(lineno); // search line
			if (lineno != getlineno(lp)) { // if not found
				err = ERR_ULN;
				break;
			}
			clp = lp; // update line pointer
			cip = clp + 3; // update i-code pointer
			break;
		case I_GOSUB:
			cip++;
			lineno = iexp(); // get line number
			if (err)
				break;
			lp = getlp(lineno); // search line
			if (lineno != getlineno(lp)) { // if not found
				err = ERR_ULN;
				break;
			}
			// push pointers
			if (gstki > SIZE_GSTK - 2) { // stack overflow ?
				err = ERR_GSTKOF;
				break;
			}
			gstk[gstki++] = clp; // push line pointer
			gstk[gstki++] = cip; // push i-code pointer
			clp = lp; // update line pointer
			cip = clp + 3; // update i-code pointer
			break;
		case I_RETURN:
			if (gstki < 2) { // stack empty ?
				err = ERR_GSTKUF;
				break;
			}
			cip = gstk[--gstki]; // pop line pointer
			clp = gstk[--gstki]; // pop i-code pointer
			break;
		case I_FOR:
			cip++;
			if (*cip++ != I_VAR) { // no variable
				err = ERR_FORWOV;
				break;
			}
			index = *cip; // get variable index
			ivar(); // var = value
			if (err)
				break;
			if (*cip == I_TO) {
				cip++;
				vto = iexp(); // get TO value
			}
			else {
				err = ERR_FORWOTO;
				break;
			}
			if (*cip == I_STEP) {
				cip++;
				vstep = iexp(); // get STEP value
			}
			else
				vstep = 1; // default STEP value
						   // overflow check
			if (((vstep < 0) && (-32767 - vstep > vto)) ||
				((vstep > 0) && (32767 - vstep < vto))) {
				err = ERR_VOF;
				break;
			}
			// push pointers
			if (lstki > SIZE_LSTK - 5) { // stack overflow ?
				err = ERR_LSTKOF;
				break;
			}
			lstk[lstki++] = clp; // push line pointer
			lstk[lstki++] = cip; // push i-code pointer
								 //Special thanks hardyboy
			lstk[lstki++] = (WCHAR*)(uintptr_t)vto; // push TO value
			lstk[lstki++] = (WCHAR*)(uintptr_t)vstep; // push STEP value
			lstk[lstki++] = (WCHAR*)(uintptr_t)index; // push variable index
			break;
		case I_NEXT:
			cip++;
			if (lstki < 5) { // stack empty ?
				err = ERR_LSTKUF;
				break;
			}
			index = (int)(uintptr_t)lstk[lstki - 1]; // read variable index
			if (*cip++ != I_VAR) { // no variable
				err = ERR_NEXTWOV;
				break;
			}
			if (*cip++ != index) { // not equal index
				err = ERR_NEXTUM;
				break;
			}
			vstep = (int)(uintptr_t)lstk[lstki - 2]; // read STEP value
			var[index] += vstep; // update loop counter
			vto = (int)(uintptr_t)lstk[lstki - 3]; // read TO value
													 // loop end
			if (((vstep < 0) && (var[index] < vto)) ||
				((vstep > 0) && (var[index] > vto))) {
				lstki -= 5; // resume stack
				break;
			}
			// loop continue
			cip = lstk[lstki - 4]; // read line pointer
			clp = lstk[lstki - 5]; // read i-code pointer
			break;
		case I_IF:
			cip++;
			condition = iexp(); // get condition
			if (err) {
				err = ERR_IFWOC;
				break;
			}
			if (condition) // if true continue
				break;
			// If false, same as REM
		case I_REM:
			// Seek pointer to I_EOL
			// No problem even if it points not realy end of line
			while (*cip != I_EOL)
				cip++; // seek end of line
			break;
		case I_STOP:
			while (*clp)
				clp += *clp; // seek end
			return clp;
		case I_VAR:
			cip++;
			ivar();
			break;
		case I_ARRAY:
			cip++;
			iarray();
			break;
		case I_LET:
			cip++;
			ilet();
			break;
		case I_PRINT:
			cip++;
			iprint();
			break;
		case I_SEMI:
			cip++;
			break;
		case I_NEW:
		case I_RUN:
			err = ERR_COM;
			break;
		default:
			err = ERR_SYNTAX;
			break;
		}
		if (err)
			return NULL;
	}
	return clp + *clp;
}

// RUN command handler
void CBasic::irun()
{
	WCHAR* lp;
	gstki = 0;
	lstki = 0;
	clp = listbuf;
	while (*clp) {
		cip = clp + 3;
		lp = iexe();
		if (err)
			return;
		clp = lp;
	}
}

//NEW command handler
void CBasic::inew(void)
{
	unsigned char i;
	for (i = 0; i < 26; i++)
		var[i] = 0;
	for (i = 0; i < SIZE_ARRY; i++)
		arr[i] = 0;
	gstki = 0;
	lstki = 0;
	*listbuf = 0;
	clp = listbuf;
}

//Command precessor
void CBasic::icom()
{
	cip = ibuf;
	switch (*cip)
	{
	case I_NEW:
		cip++;
		if (*cip == I_EOL)
			inew();
		else
			err = ERR_SYNTAX;
		break;
	case I_RUN:
		cip++;
		irun();
		break;
	default:
		iexe();
		break;
	}
}

// Print OK or error message
void CBasic::error()
{
	if (err)
	{
		if (cip >= listbuf && cip < listbuf + SIZE_LIST && *clp)
		{
			SendMessage(m_hOutputEditWnd, EM_REPLACESEL, 0, (LPARAM)L"LINE:");
			putnum(getlineno(clp), 0);
			SendMessage(m_hOutputEditWnd, EM_REPLACESEL, 0, (LPARAM)L" ");
			putlist(clp + 3);
		}
		else
		{
			newline();
			c_puts(L"YOU TYPE: ");
			c_puts(lbuf);
		}
	}
	newline();
	c_puts(errmsg[err]);
	newline();
	err = 0;
}

void CBasic::init()
{
	m_bAbort = FALSE;
	clp = NULL;
	cip = NULL;
	gstki = 0;
	lstki = 0;
	err = 0;
	ZeroMemory(lbuf, sizeof(lbuf));
	ZeroMemory(ibuf, sizeof(ibuf));
	ZeroMemory(var, sizeof(var));
	ZeroMemory(arr, sizeof(arr));
	ZeroMemory(listbuf, sizeof(listbuf));
	ZeroMemory(gstk, sizeof(gstk));
	ZeroMemory(lstk, sizeof(lstk));
}

void CBasic::Run()
{
	init();
	SetWindowText(m_hOutputEditWnd, 0);
	BOOL bError = FALSE;
	DWORD dwSize = GetWindowTextLengthW(m_hInputEditWnd);
	LPWSTR lpszText = (LPWSTR)GlobalAlloc(0, sizeof(WCHAR) * (dwSize + 1));
	GetWindowTextW(m_hInputEditWnd, lpszText, dwSize + 1);
	LPCWSTR lpszDelimiter = L"\r\n";
	LPWSTR context = 0;
	LPWSTR token = wcstok_s(lpszText, lpszDelimiter, &context);
	while (token && !bError)
	{
		LPWSTR szText = (LPWSTR)GlobalAlloc(0, (lstrlenW(token) + 1) * sizeof(WCHAR));
		lstrcpyW(szText, token);
		StrTrimW(szText, L" ");
		{
			lstrcpy(lbuf, szText);
			WCHAR len = toktoi(); // Convert token to i-code
			if (err) { // Error
				error(); // Print error message
				bError = TRUE;
			}
			else if (*ibuf == I_NUM) { // Case the top includes line number
				*ibuf = len; // Change I_NUM to byte length
				inslist(); // Insert list
				if (err) {
					// Error
					error();  // Print error message
					bError = TRUE;
				}
			}
		}
		GlobalFree(szText);
		token = wcstok_s(0, lpszDelimiter, &context);
	}
	GlobalFree(lpszText);
	if (!bError)
	{
		irun();
	}
}

void CBasic::newline(void)
{
	SendMessage(m_hOutputEditWnd, EM_REPLACESEL, 0, (LPARAM)TEXT("\r\n"));
}
